...
function validateEntry(element, previousElementId, wordList) {
  ...
  finalElement = document.getElementById('word_final');
  isFinalWord = false;
  finalWord = finalElement.value;
  previousWord = previousElement.value;
  testId = 'word' + lpad(parseInt(element.id.substring(4, 6)) + 1, 2);
  ...
  if (isFinalWord && levenshtein(finalWord, wordEntered) != 1) {
    window.alert('The word must differ by exactly one letter from the final word.');
    element.value = '';
    element.focus();
  }
}