...
var sample = false;
var images = [[imageComma, commaURL], [imageSemicolon, semicolonURL], [imageHyphen, hyphenURL], [imageFullstop, fullstopURL], [image66, URL66], [image99, URL99], [imageApostrophe, apostropheURL], [imageBracketLeft, bracketLeftURL], [imageBracketRight, bracketRightURL], [imageQuestionMark, questionMarkURL], [imageForwardSlash, forwardSlashURL], [imageExclamationMark, forwardSlashURL]];
var dropDowns = [];
var r = "";
...
function showHideDropDown(id, y, d) {
  ...
  $a.css('top', pos + mouse.y - 35 + "px"); //////console.log(d);
}

function createDropDown(obj, id, y) {
  var exercise = "exercise_" + id;
  var create = '<div id="' + exercise + "_" + y + '_d" class="dropBlock">'; //  create += '<li><a onclick="javascript:showlayer('exercise')"> Profile</a></li>;
  ...
  for (var i = 0; i < obj.length; i++) {
    ...
    mytext = encodeURIComponent(mytext); //////console.log("MYTEXT = "+mytext);
  ...
  var $ii = $('#gameCanvas' + id);
  var pos = $ii.position().top + 50; //////console.log("pos = "+pos);
  //////console.log("y = :"+y);

  var $a = $("#" + exercise + "_" + y + "_d");
  var newY = $a.position().top - pos; //////console.log($a.position());
  ...
  $a.css('display', 'none');
  zindex = zindex + 1; //////console.log($a.position());
...
function createOptions(obj, id, y, answer) {
  ...
  create += '</select></div>';
  jQuery('#' + exercise).append(create); // position DIV
  //  var $b = $('#'+id);
  //$a.css('position', 'absolute');

  var $ii = $('#gameCanvas' + id); //////console.log(id+" pos = "+$ii.position().top);
...
var changeQuotes = false; /// import the game objects

function gameCodeInit(obj, res, myobs, _sample) {
  //alert("test 2");
  if (_sample == true) sample = true;
  submitFunction = myobs; //  alert("role = "+role);
  ...
  isviewable = Number(viewable); // alert(">"+isviewable);
  ...
  if (res.length == 20 && isviewable == 2) {
    var correctAnswers = 0; // find out how many correct answers there are
  ...
  var myColor = "#FF0000";
  var myFont = "Arial";
  var mySize = 14; // var test = new display.newText(obj[i].exercise_type,150,50,myFont,mySize,myColor);
...
var loader = new PxLoader(),
    ...
    imageForwardSlash = loader.addImage(forwardSlashURL); // callback that will be run once images are ready 
...
function setUpScenarios() {
  ...
  if (viewAsStudent == "yes") role = "Admin"; // THIS WILL CHECK IF THE ANSWER HAS ALREADY BEEN ANSWERED OR NOT

  qnum = 0;

  for (var i = 0; i < exercises.length; i++) {
    // set the canvas to the current exercise
    gameCanvas = canvases[i]; //////console.log("*>"+exercises[i][0]);
    ...
    switch (exercises[i][0]) {
      ...
      case "add_words":
        if (!checkAnswerArray(qnum)) setUpAddWords(i);else drawAnswer(i);
        break;

      case "underline_circle_highlight":
        if (!checkAnswerArray(qnum)) setUnderlineCircleHighlight(i);else drawAnswer(i);
        break;

      case "move_characters":
        if (!checkAnswerArray(qnum)) setUpMoveCharacters(i);else drawAnswer(i);
        break;

      case "custom_choice":
        if (!checkAnswerArray(qnum)) setUpCustomChoice(i);else drawAnswer(i);
        break;
...
function setUpSpellingTest(i) {
  ...
  var pos = $ii.position().top; // position DIV
  ...
  var maxRows = 3;
  var rowheight = 30;
  var wordY = 0; //////console.log("set up spelling");
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    test.addEventListener(Event.CLICK, testSpelling, {
      params: []
    });
    test.num = i;
    rowY++;
  ...
  gameCanvas.height = 130; //  addButton(99);
}

function addButton(i) {
  ////console.log("ADDING BUTTON,"+i);
  if (role == "Admin" && sample == false) {
    if (i != 0) block = new display.newRect(5, 5, 10, 10, blockColor);else block = new display.newText("SUBMIT", 10, 5, "Arial", 10, "#FF0000");
    block.i = i;
    block.addEventListener(Event.CLICK, checkButton, {
      params: []
    });
  }
}

function checkAll(e) {//console.log("check all");
}
...
function removeListenersFromCanvas(i) {
  ...
  if (iphone || ipad) {
    ...
    gameCanvas.removeEventListener('touchend', mouseUp, false); //  $(gameCanvas).bind('touchmove',mouseMove);
...
function checkButton(e) {
  refreshPage = true; // if(completed == 0){  
  //////console.log(e);
  //if(e.parent.i == 99){
  //magnify = !magnify;
  ...
  valid = true; //////console.log("clicked button - "+e.parent.i,"answer array = "+answerArray);
...
function checkAnswer_underlineCircleHighlight(ob, num) {
  ...
  for (var i = 0; i < ar.length; i++) {
    ...
    if (ob.questions[i].highlight != "") highlightArray2 = ob.questions[i].highlight.split("+")[1].split(" "); //////console.log("underline array = ",underlineArray2);
    ...
    for (var j = 0; j < ar[i].length; j++) {
      ...
      var highlightFound = false; // lets add the block string to our answers string

      blockString = blockString + ar[i][j].text + "*";

      for (var u = 0; u < underlineArray2.length; u++) {
        // NOW LOOP THROUGH UNDERLINE ARRAY 
        if (underlineArray2[u] == ar[i][j].num) {
          // IF UNDERLINE ARRAY NUM = THE SAME AS THE WORD NUM
      ...
      if (ar[i][j].underline) {
        underlineString = underlineString + (j + 1) + " "; //prevUnder = ar[i][j].num;
      } //-----------


      for (u = 0; u < circleArray2.length; u++) {
        // NOW LOOP THROUGH UNDERLINE ARRAY 
        if (circleArray2[u] == ar[i][j].num) {
          // IF CIRCLE ARRAY NUM = THE SAME AS THE WORD NUM
      ...
      if (ar[i][j].showRect) {
        circleString = circleString + (j + 1) + " "; //prevCirc = ar[i][j].num;
      } //---------------


      for (u = 0; u < highlightArray2.length; u++) {
        // NOW LOOP THROUGH UNDERLINE ARRAY 
        if (highlightArray2[u] == ar[i][j].num) {
          // IF CIRCLE ARRAY NUM = THE SAME AS THE WORD NUM
      ...
      if (ar[i][j].highlight) {
        highlightString = highlightString + (j + 1) + " "; //prevHigh = ar[i][j].num;
      } // //////console.log("num > "+ar[i][j].num);

    }

    var correct = true; //////console.log("---------");

    if (!underlineAnswer || !circleAnswer || !highlightAnswer) correct = false; //  if(underlineString != underlineArray2)correct = false;
    ...
    if (correct) correct = 1;else correct = 0; // CHECK VALID

    if (underlineString == "" && circleString == "" && highlightString == "" && role != "Admin") valid = false; // SET UP ANSWER APP   ---------------------

    answerOb.exercise = 0; // answerOb.question = answerArray.length+1;
...
function checkAnswer_movecharacters(ob, num) {
  ...
  for (var i = 0; i < ar.length; i++) {
    ...
    var answerStr = str; // take out all spaces

    str = str.replace(/\s/g, '');
    str2 = str2.replace(/\s/g, '');
    str = cleanString(str);
    str2 = cleanString(str2); //////console.log("------");
    ...
    answerOb.exercise = 1; // answerOb.question = answerArray.length+1;
...
function checkAnswer_customchoicedetailed(ob, num) {
  var ar = ob.array;
  var answerOb;
  var answerStr; //////console.log(">>> "+ob.questions.length);
  ...
  for (var i = 0; i < ar.length; i++) {
    ...
    if (ob.questions[i].word == "new") str2 = ob.questions[i].answer; // take out all spaces
    ...
    if (correct) {
      if (role == "Admin") ar[i][0].block.set({
        color: lightGreenColor
      });
      correct = 1;
    } else {
    ...
    if (answerStr.split(getDots(5)).length != 1 && role != "Admin") valid = false; // check valid

    if (ob.questions[i].word != "new" && (opt == "null" || opt == null) && role != "Admin") valid = false; // SET UP ANSWER APP   ---------------------

    answerOb.exercise = 2; // answerOb.question = answerArray.length+1;
...
function checkAnswer_customchoice(ob, num) {
  ...
  for (var i = 0; i < ar.length; i++) {
    ...
    var correct = true;
    underlineString = "";
    blockString = ""; // first check drop down box

    var opt = checkOptions("opt_" + ob.opArray[i][0] + "_" + ob.opArray[i][1]);
    if (ob.questions[i].correct_item != opt) correct = false; //////console.log("-> "+ob.questions[i].correct_item);
    ...
    for (var j = 0; j < ar[i].length; j++) {
      ...
      if (ar[i][j].text != "") {
        blockString = blockString + ar[i][j].text + "*"; // str = str+ar[i][j].text;

        var underlineArray;
        var underlineFound = false; //////console.log("num = "+ar[i][j].num);
        //////console.log("<> "+ar[i][j]);

        for (var u = 0; u < underlineArray.length; u++) {
          // NOW LOOP THROUGH UNDERLINE ARRAY 
          if (underlineArray[u] == ar[i][j].num) {
            // IF UNDERLINE ARRAY NUM = THE SAME AS THE WORD NUM
    ...
    underlineString = underlineString + "+" + opt; ////////console.log(">"+str);
    // //////console.log("------");
    ...
    if ((opt == "null" || opt == null) && role != "Admin") valid = false; // SET UP ANSWER APP   ---------------------

    answerOb.exercise = 3; // answerOb.question = answerArray.length+1;
...
function checkAnswer_customchoice_old(ob, num) {
  ...
  for (var i = 0; i < ar.length; i++) {
    ...
    var str = "";
    underlineString = ""; ////////console.log("> "+checkOptions("opt_"+ob.opArray[i][0]+"_"+ob.opArray[i][1]));
    // //////console.log(">>"+ob.questions[i].correct_item);
    ...
    var correct = true;
    if (underlineString != testUnderlineString) correct = false;
    if (ob.questions[i].correct_item != checkOptions("opt_" + ob.opArray[i][0] + "_" + ob.opArray[i][1])) correct = false; //////console.log("------");
    //////console.log("> underline "+underlineString);
    //////console.log("> test-underline "+testUnderlineString);
    ...
    answerOb.exercise = 44; // answerOb.question = i+1;
...
function checkAnswer_addwords(ob, num) {
  ...
  for (var i = 0; i < ar.length; i++) {
    ...
    var str2 = ob.questions[i].answer; // take out all spaces
    ...
    if (answerStr.split(getDots(5)).length != 1 && role != "Admin") valid = false; // SET UP ANSWER APP   ---------------------

    answerOb.exercise = 4; // answerOb.question = answerArray.length+1;
...
function drawAnswer(i) {
  gameCanvas = canvases[i]; // get rid of save button

  jQuery('#submit-text' + (i + 1)).html("");
  jQuery('#submit-text' + (i + 1)).hide(); //console.log("drawing answer");
  ...
  qnum = qnum + questions.length;
  var num = i; //var topPad = 50;

  var rowY = 0;
  var rowX = leftPad;
  var column = 1;
  var columns = 4; //var rowheight = 30;
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    myColor = blackColor; // add grey line for visibility

    var myBlock = addBlock(0, wordY - 10, i); // check to show if correct or not
    ...
    if (showBlock == true) {
      ...
      if (splitAsterisk) mySplitBlock = answerArray[rowNumberA].answer.split("+")[0].split("*"); //////console.log("mySplitBlock = ",mySplitBlock);
      ...
      for (var sb = 0; sb < mySplitBlock.length; sb++) {
        ...
        if (mySplitBlock[sb] != "") {
          ...
          myspace.row = false;
          rowX = rowX + myspace.width; /// NOW we have to add a SPACE
          ...
          if (rowX > maxWidth + 90) {
            ...
            myspace.row = true; //  myBlock.set({height:(rowheight*2)});

            myBlock.set({
              height: wordY + rowheight
            });
          }
        }
      }
      ...
      wordY = wordY + rowheight; // end check add second row
    } // now..  loop through blocks and see if we should underline.


    if (type == "custom_choice") {
      var underlineArray = answerArray[rowNumberA].answer.split("+")[1].split(" "); //////console.log("****************");
      //////console.log("****************");

      var userno = false;
      if (questions[i].underline.split("+").length > 2) if (questions[i].underline.split("+")[2] == "no") userno = true; //////console.log("underline array = ",underlineArray,userno);
...
function setUpAddWords(i) {
  ...
  addButton(i);
  var dragArray = new Array();
  var wordX = leftPad; // set up drag boxes
  ...
  for (var i = 0; i < questions.length; i++) {
    var myOb = splitObs(questions[i]); //////console.log("*********************");
    ...
    for (var s = 0; s < myOb.decoys.length; s++) {
      if (myOb.decoys[s] != "" && myOb.decoys[s] != " ") myAnswers.push(myOb.decoys[s]); //////console.log(myOb.decoys[s]);
    }
  } //////console.log("*********************");
  // randomise the array
  ...
  if (!role || role == "Admin") {
    ...
    for (i = 0; i < myAnswers.length; i++) {
      ...
      smallword.addEventListener(Event.CLICK, showRect, {
        params: []
      });
      smallword.addEventListener(Event.RELEASE, checkDragDrop, {
        params: []
      });
      smallword.isDraggable = true; // test.addEventListener(Event.RELEASE,highlightText,{params:[]});
      //test2.num = i;
      //test2.setText(
      // questions[i].exercise_textbox = test;

      wordX = smallword.x + smallword.width + 20; //////console.log(wordX);
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    var splitArray = questions[i].answer.split(splitword); //var questions[i].my = new Object();
    ...
    emptyOb.array.push(myBlockArray); //questions[i].my.array = myBlockArray;
    // lets add the words taken from the sentence
    ...
    for (var s = 0; s < myOb.blocks.length; s++) {
      ...
      for (var n = 0; n < myOb.nums.length; n++) {
        ...
        if (s == myOb.nums[n] - 1) {
          myColor = greyDots; //   var myNum = myOb.blocks[s].length+(Math.random()*3);
          //  droptext = "[ ";
          ...
          droptarget.block = myBlock; // var widthText = new display.newText(myOb.blocks[s],rowX,wordY,myFont,mySize,myColor);

          rowX = rowX + droptarget.width;
          dragArray.push(droptarget);
          showBlock = false;
          myBlockArray.push(droptarget);
          droptarget.blockArray = myBlockArray; // check if gone too far - add second row

          if (rowX > maxWidth + 90) {
            rowX = leftPad;
            rowY++; //  wordY = topPad+(rowY*rowheight)-10;
            ...
            droptarget.row = true;
            rowX = rowX + droptarget.width; //  myBlock.set({height:(rowheight*2)});
      ...
      if (showBlock == true) {
        myColor = blackColor; // clean the string
        ...
        */
        ...
        for (var sb = 0; sb < mySplitBlock.length; sb++) {
          ...
          myBlockArray.push(myspace); /// NOW we have to add a SPACE
          // check if gone too far - add second row
...
function realignBlocks(ar) {
  var rowX = leftPad; // realign all text
  ...
  for (var s = 0; s < ar.length; s++) {
    ...
    rowX = rowX + ar[s].width; //////console.log("> "+ar[s].text+" - "+ar[s].row);
    ...
    if (ar[s].taken) ar[s].symbiote.set({
      x: ar[s].x
    });
...
function checkCharacter(t, t2) {
  ...
  if (t.text == "!") val = true;
  if (t.text == ".") val = true;
  if (t.text == ",") val = true; // check previous word
...
function setUnderlineCircleHighlight(i) {
  ...
  var rowY = 0;
  var column = 1;
  var columns = 4; //var rowheight = 20;
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    var myBlock = addBlock(0, wordY - 10, i); //clean text
    ...
    splitArray = questions[i].sentence.split("*"); //////console.log("sentence = "+ questions[i].sentence);
    ...
    for (var j = 0; j < splitArray.length; j++) {
      ...
      if (questions[i].highlight != "") if (questions[i].highlight.split("+")[0] == "B") blockselect = true; // //////console.log("BLOCK SELECT  -"+blockselect);
      ...
      for (k = 0; k < newSplitArray.length; k++) {
        ...
        if (newSplitArray[k] != "" && (!role || role == "Admin")) test.addEventListener(Event.CLICK, highlightText2, {
          params: []
        });
        test.num = mynum;
        test.highlightArray = highlightArray;
        test.block = myBlock;
        textX = textX + test.width + 5; // IF TEACHER, HIGHLIGHT TEXT
        ...
        if (role && role != "Admin") {
          ...
          if (questions[i].highlight != "") highlightArray = questions[i].highlight.split("+")[1].split(" "); //  for(b = 0;b<blockArray.length;b++){
        ...
        ;
        prevtest = test; // check if gone too far - add second row

        if (textX > maxWidth + 60 && k != newSplitArray.length - 1) {
          textX = leftPad;
          rowY++;
          wordY = topPad + rowY * rowheight - 10; // myspace.set({x:leftPad,y:wordY});
          //myspace.row = true;

          if (myBlock) myBlock.set({
            height: rowheight * 2
          });
        }
    ...
    emptyOb.array.push(dragArray); // lets do this
...
function setUpCustomChoice(i) {
  ...
  var wordY = 0;
  var highlightArray = []; //var myOb = splitObs(questions[i]);
  ...
  emptyOb.array = new Array();
  myExercises.push(emptyOb); /// NEW CODE ***********************************************************************

  for (var i = 0; i < questions.length; i++) {
    wordY = topPad + rowY * rowheight; //    var splitword = questions[i].word.split("*")[0];
    ...
    myOb.user = "no"; // check for decoys

    if (myArray[2]) myOb.user = myArray[2];
    myColor = blackColor;
    var user = false; // add the underline option to the highlight array
    ...
    var myBlock = addBlock(0, wordY - 10, i); //  var myOb = splitObs(questions[i]);
    ...
    var myBlockArray = [];
    var mynum = 1;
    var newSplitArray; //  if(myOb.user == "user")myOb.blocks = questions[i].sentence.split(" ");
    // lets add the words taken from the sentence
    ...
    for (var s = 0; s < myOb.blocks.length; s++) {
      ...
      if (myOb.user != "user") {
        ...
        // myOb.blocks[s] = myOb.blocks[s].replace(/&#039;/gi,"'");
        myOb.blocks[s] = cleanString(myOb.blocks[s]);
        newSplitArray = myOb.blocks[s].split(" "); // myOb.blocks[s] = myOb.blocks[s].replace(/&quot;/gi,"''");
        ...
        for (k = 0; k < newSplitArray.length; k++) {
          ...
          rowX = rowX + test3.width;
          test3.block = myBlock;
          test3.num = mynum; //test3.num  = s;

          myBlockArray.push(test3); // CHECK if the character is a full stop and space needs to be adjusted
          ...
          if (test3.text != "" && (!role || role == "Admin")) {
            test3.addEventListener(Event.CLICK, highlightText2, {
              params: []
            });
            test3.highlightArray = highlightArray;
    ...
    emptyOb.array.push(myBlockArray); // add optoins
...
function setUpNewGame(i) {
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    var splitnum = [];
    if (split.length == 2) splitnum = split[1].split(" "); //////console.log("-----");
    //////console.log(split);
    //////console.log(splitnum);
    //////console.log("-----");

    var splitArray;
    if (splitword.length == 1) splitArray = questions[i].sentence.split(" ");else splitArray = splitword;
    ...
    var showBlock;
    var myBlockArray = [];
    emptyOb.array.push(myBlockArray); // lets add the words taken from the sentence
    ...
    for (var s = 0; s < myOb.blocks.length; s++) {
      ...
      if (showBlock == true) {
        ...
        rowX = rowX + test3.width;
        myBlockArray.push(test3);
        test3.blockArray = myBlockArray; // check if we should add a button click to a word
        ...
        if (shouldaddButton) test3.addEventListener(Event.CLICK, changeTextDropDown2, {
          params: []
        });
        test3.selectName = 'opt_' + (num + 1) + '_' + wordY;
        test3.num = num + 1;
        test3.i = i + 1; // check to add a space after...

        var showspace = true; //first.. let print out the next character..

        if (myOb.blocks[s + 1]) {
          var nextChar = myOb.blocks[s + 1]; //////console.log("nextchar = "+nextChar[0]);
        ...
        if (showspace) {
          ...
          myspace.blockArray = myBlockArray; // check if gone too far - add second row
...
function changeTextDropDown2(e) {
  ...
  showHideDropDown(e.parent.num, e.parent.i, "block"); //    document.getElementById('topDrop').innerHTML = e.parent.resetText;
  //  alert(e.parent.num);
}
...
function setUpCustomChoiceDetailed(i) {
  ...
  for (var i = 0; i < questions.length; i++) {
    var myOb = splitObs(questions[i]); // lets add the words taken from the sentence
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    var droptext = "";
    var showBlock;
    var myBlockArray = []; // lets add the words taken from the sentence
    ...
    createOptions(newArray, num + 1, wordY, answer); // lets do this
...
function setUpMoveCharacters(i) {
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    var tempArray = questions[i].character.split("*"); //////console.log(tempArray);

    for (var t = 0; t < tempArray.length; t++) {
      var tinyTemp;
      if (tempArray.length > 1) tinyTemp = tempArray[t].split("+")[1].split(" ");else tinyTemp = tempArray[t].split("+")[1].split(" ");
      ;

      for (var tt = 0; tt < tinyTemp.length; tt++) {
        characters.push({
          ar: tempArray[t].split("+"),
          "char": tempArray[t].split("+")[0],
          num: i
        });
      }
    }
  ...
  if (!role || role == "Admin") {
    // IF TEACHER, DON"T SHOW THIS
    for (c = 0; c < characters.length; c++) {
      myColor = blueLightColor;
      mySize = 16; //mychar = new display.newText(characters[c],markerX,10,myFont,mySize,myColor);

      var url;
      var src; // characters[c].char = characters[c].char.replace(/\s/g,'');
      ...
      mychar.addEventListener(Event.CLICK, dragWord, {
        params: []
      });
      mychar.addEventListener(Event.RELEASE, checkJoinDrop2, {
        params: []
      }); //mychar.showRect = true;
      //test.isDraggable = true;

      mychar.lockY = false; // mychar.set({x: markerX, y:10});
      ...
      markerX = markerX + 30;
      if (characters[c].ar.length > 2) if (characters[c].ar[2] == "add") mychar.add = true; // check if gone too far right..
  ...
  for (var i = 0; i < questions.length; i++) {
    myColor = blackColor;
    twoRows = false; // NEW CODE

    var characterOb = new Object();
    var myArray = questions[i].character.split("*");
    var push = false; // var mysentence  = questions[i].sentence;
    ...
    if (!role || role == "Admin") {
      ...
      for (var cc = 0; cc < myArray.length; cc++) {
        // then loop through amounts character types
        foundCharacter = 0;
        ...
        mysentence = ""; //////console.log("mytemparray = "+tmpArray);
    ...
    for (var j = 0; j < splitArray.length; j++) {
      ...
      mytext = cleanString(mytext); // add a block at start
      ...
      test.block = myBlock;
      dragArray.push(test); //  test.markerArray = new Array();

      rowX = rowX + test.width + 1;
      test.i = j;
      test.row = false;
      test.rowNum = dar;
      test.array = dragArray; //////console.log(">"+mytext+"<");
      // check if gone too far - add second row
      ...
      if (rowX > maxWidth && mytext[0] == " " && j != splitArray.length - 1) {
        ...
        dar = 2;
        rowX = leftPad;
        rowY = rowY + topPad + rowheight / 3 + 10; //rowY = topPad+(rowNum*(rowheight/2))+60;
        //  wordY = topPad+(rowY*rowheight)
    ...
    for (var ma = 0; ma < markerArray.length; ma++) {
      if (markerArray[ma].num == i && markerArray[ma].add == true) {
        ...
        */
        ...
        if (rand == markerArray.length) rand = markerArray.length - 2;
        markerArray[ma].i = rand; //////console.log(markerArray[ma].i);

        markerArray[ma].array = dragArray;
        markerArray[ma].ar = dragArray; //dragMarker.i = j;
        ...
        dragArray.splice(rand, 0, markerArray[ma]); // markerArray[ma].set({x:newX});
        // refreshArray(dragArray,99);
    ...
    rowNum++; // // if(!twoRows)rowY = rowY +(rowheight);
    // else rowY = rowY +(rowheight/3);

    rowY = rowY + rowheight;
    refreshArray(dragArray, 99);
    dragArrays.push(dragArray); // questions[i].exercise_textbox = test;
...
function testSpelling(e) {
  spellingWord = e.parent; // position DIV
  ...
  $b.show();
  $b.val("");
  $b.css('display', 'block'); // HIDE TEXT BUTTON

  $("#button").css('display', 'none');
  setTimeout(function () {
    $('input[name="spellingText"]').focus();
  }, 100); //var input = document.getElementById("spellingText").focus();
  //$('#spellingText').select()
  // set up temp text

  var $s = $("#hintText");
  $s.css('display', 'block'); //var str = $("p:first").text();
...
function checkSpelling() {
  ...
  word = word.toLowerCase(); //////console.log(word,spellingWord.text);

  if (word == spellingWord.text) {
    spellingWord.fontColor = "#0C0";
    $("#spellingDIV").css('display', 'none'); //////console.log("correct");
  } else {
    spellingWord.fontColor = redColor;
    $("#spellingDIV").css('display', 'none'); //////console.log("WRONG");
  }
...
function cleanString(str) {
  str = str.replace(/&#039;/g, "'"); ////   str = str.replace(/&quot;/gi,'"');
  // replace for 
  //str = str.replace(/\./g,' ')
  // check for PLUS and change to +

  str = str.replace(/PLUS/g, '+'); // str = str.replace(/%26%23039%3B/gi,"'");
...
function stopDragWord() {// if(dragMarker)//////console.log(dragMarker.x,dragMarker.y);
...
function redrawArray(ar, insert, j) {
  var textX = 50; //////console.log("fffff");
...
function myDragLoop() {
  ...
  var $a = $("#textContainer"); // //////console.log($a, $("#textContainer").attr("spellcheck","false"));
    ...
    for (i = 0; i < dragArrays.length; i++) {
      if (dragArrays[i][0].canvas == gameCanvas) {
        for (var j = 0; j < dragArrays[i].length; j++) {
          ...
          rect.rect.center.y = rect.y + rect.height / 2; // check the whitespace between letters
          ...
          dc.y = dragMarker.rect.center.y; // if(checkRect(dragArrays[i][j],dc.x,dc.y) || checkRect(ob,dc.x,dc.y)){
          ...
          if (checkRect(dragArrays[i][j], dc.x, dc.y)) {
            ...
            hit = true;
            dragMarker.array = dragArrays[i]; //dragMarker.i = j;
          }
        }

        if (!hit) refreshArray(dragArrays[i], 99);
        hit = false; // //////console.log(dragArrays[0].length,dragArrays.length);
      }
    }
...
game.loops.push(myDragLoop); // redraw array
...
function refreshArray_(ar, j) {
  ...
  for (var i = 0; i < ar.length; i++) {
    ...
    for (var k = 0; i < ar[i].length; k++) {
      var ob = ar[i][k]; //////console.log("OB = "+ob);
  ...
  for (var i = 0; i < ar.length; i++) {
    ...
    if (i != j) {
      ...
      if (ar[i].text == endlineMarker) {
        textX = templeftPad; //  ar[i].set({x:textX});
      }
    } else {
      // insert.set({x:textX,y:ar[0].y});
      //if(j!=99) textX = textX+dragMarker.width+10;
...
function checkJoinDrop2(e) {
  if (e.parent.array) {
    if (!e.parent.pinned) {
      ...
      var ar = e.parent.array; // //////console.log(ar);
      ...
      if (foundchar == true) {
        ar.splice(e.parent.i, 0, e.parent); //////console.log(">drop into position >"+e.parent.i+" - and array length = "+ar.length);
    ...
    } else {//////console.log("pinned = "+e.parent.pinned);
      //resetMoveCharPos(e);
      //alert("nice tryâ€¦ Jim");
    }
  } else {
...
function resetMoveCharPos(e) {
  e.parent.showRect = true; //if(pinned)removeMarker(e.parent);
...
function insertIntoSentence(_char) {
  _char.showRect = false;
  var ar = _char.array; //////console.log(ar);
  // e.parent.set({text:"Good job!"});
  // e.parent.set({x:e.parent.dragTarget.rect.center.x-(e.parent.width/2)});
  // e.parent.set({y:e.parent.dragTarget.rect.center.y-(e.parent.height/2)});
  // ar.splice(e.parent.i,1);
  ...
  ar.splice(_char.i, 0, _char); //////console.log(ar);
...
function checkDragDrop(e) {
  var drop = false; // loop through all the spots to drop the text
  ...
  if (drop) {
    ...
    e.parent.showRect = false; // e.parent.set({x:e.parent.dragTarget.rect.center.x-(e.parent.width/2)});
    //e.parent.set({y:e.parent.dragTarget.rect.center.y-(e.parent.height/2)});

    e.parent.set({
      x: e.parent.dragTarget.x
    });
    e.parent.set({
      y: e.parent.dragTarget.y
    });
    e.parent.dragTarget.set({
      text: e.parent.text
...
function checkJoinDrop(e) {
  if (checkRect(e.parent.dragTarget, e.parent.rect.center.x, e.parent.rect.center.y)) {
    e.parent.showRect = false; // e.parent.set({text:"Good job!"});

    e.parent.set({
      x: e.parent.dragTarget.rect.center.x - e.parent.width / 2
    });
    e.parent.set({
      y: e.parent.dragTarget.rect.center.y - e.parent.height / 2
    });
  } else {
    e.parent.set({
      x: e.parent.orginX
...
function showRect(e) {
  e.parent.index = display.getNewIndex();
  display.sortIndex(); // check to release dragTarget
...
function checkReleaseDragTarget(text) {
  ...
  if (dragTarget) {
    ...
    if (dragTarget.taken) {
      dragTarget.taken = false; //  alert(dragTarget.resetText);

      dragTarget.set({
        text: dragTarget.resetText
      });
...
function highlightText(e) {
  //e.parent.showRect = true;
  if (e.parent.highlight == false) e.parent.highlight = true;else if (e.parent.highlight) {
    e.parent.highlight = false;
  }
  e.parent.set({});
}
