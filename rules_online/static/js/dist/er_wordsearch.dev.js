  ...
  */
  ...
  var WordFind = function WordFind() {
    ...
    */
    ...
    var allOrientations = ['horizontal', 'horizontalBack', 'vertical', 'verticalUp', 'diagonal', 'diagonalUp', 'diagonalBack', 'diagonalUpBack']; // The definition of the orientation, calculates the next square given a
    ...
    var findBestLocations = function findBestLocations(puzzle, options, word) {
      ...
      for (var k = 0, len = options.orientations.length; k < len; k++) {
        var orientation = options.orientations[k],
            ...
            x = 0,
            y = 0; // loop through every position on the board
    ...
    */
    ...
    var calcOverlap = function calcOverlap(word, puzzle, x, y, fnGetSquare) {
      var overlap = 0; // traverse the squares to determine if the word fits
    ...
    */
    ...
    return {
      ...
      */
      newPuzzle: function newPuzzle(words, settings) {
        var wordList,
            ...
            attempts = 0,
            opts = settings || {};
        console.log('newPuzzle() :: settings = ', settings); // copy and sort the words by length, inserting words into the puzzle
      ...
      * @api public
      */
      fillBlanks: function fillBlanks(puzzle) {
...
(function (document, $, wordfind) {
  ...
  */
  ...
  var WordFindGame = function WordFindGame() {
    ...
    */
    ...
    var drawPuzzle = function drawPuzzle(el, puzzle) {
      ...
      for (var i = 0, height = puzzle.length; i < height; i++) {
        ...
        var row = puzzle[i];
        output += '<div>'; // for each element in that row
    ...
    */
    ...
    var drawWords = function drawWords(el, words) {
      var output = '<ul>';
    ...
    */
    ...
    var select = function select(target) {
      ...
      var orientation = calcOrientation($(lastSquare).attr('x') - 0, $(lastSquare).attr('y') - 0, $(target).attr('x') - 0, $(target).attr('y') - 0); // if the new square isn't along a valid orientation, just ignore it.
    ...
    */
    ...
    return {
      ...
      */
      create: function create(words, puzzleEl, wordsEl, options) {
        wordList = words.slice(0).sort();
        var puzzle = wordfind.newPuzzle(words, options);
        console.log('puzzle = ', puzzle); // draw out all of the words

        drawPuzzle(puzzleEl, puzzle);
        drawWords(wordsEl, wordList); // attach events to the buttons
...
function convertDjangoListToArray(djangoList) {
  return djangoList.replaceAll("[", "").replaceAll("]", "").replaceAll("'", "").replaceAll(" ", "").split(",");
}

$(function () {
  var words = convertDjangoListToArray(document.getElementById("wordsList").value); // start a word find game
