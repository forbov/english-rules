...
var mouse = new Object();
 ...
 game.loops = new Array();
  ...
  var mouse = {x:0,y:0}
  var drag = {isDragging:false,target:null};
...
function initCanvas(canvas){
      ...
      //writeText("no obj! "+canvas,50,250,myFont,myColor)
...
iphone = ((window.navigator.userAgent.match('iPhone'))||(window.navigator.userAgent.match('iPod')))?true:false;
  ...
  function myLoop(){
      ...
      /// check for loops pushed into this loop
  ...
  function loopDisplayObjects(){
      ...
      // var ar = display.objects.slice();; // create a clone of the display array
       
       var ar = display.objects;
       
       // loop through array to see if there's been any changes
    ...
    // }
     ...
     var ob;
      ...
      for(var i = 0;i<ar.length;i++){
          ...
          //if(ob.type == "text" || ob.type == "rectangle"){
...
if(drag.target.lockY)var newY = drag.target.y;
else newY = mouse.y-mouse.clickY;

//drag.target.set({x:mouse.x-mouse.clickX,y:newY});
////console.log(drag.target.rect.width);
drag.target.set({x:mouse.x-((drag.target.rect.width)/2),y:newY});
...
$('#magnifyDiv').css('display','block');
//$('#magpicDiv').css('display','block');

    var $ii = $('#'+gameCanvas.id);
      
      //console.log($ii,gameCanvas.id);
...
//Draw the Circle Mask.
      ...
      ctx2.beginPath();
      // (centerX, centerY, radius, starting angle, end angle, counter clockwise)
      ctx2.arc(70,70,70,0,Math.PI*2 , false);
  //  ctx2.fill();
...
//ctx2.putImageData(imgData,0,0,140,140);
ctx2.scale(2,2);
...
/// CHECK HOW MUCH TIME HAS PASSED SINCE CLICKING
...
if(e.offsetX) {
  ...
  if ((iphone)||(ipad)){ //iPad
      ...
      //clickX.y = e.touches[0].clientY- totalOffsetY;;
          ...
          clickX.y = e.targetTouches[0].pageY - totalOffsetY;
...
checkMouseListener({x:mouse.x,y:mouse.y}, Event.CLICK,e);
...
//event.preventDefault();  // stop iphone screen from scrolling
...
display.newText = function(text , x , y, font, fontSize, fontColor, returnFunction){
  ...
  this.rect = new Rectangle({x:x,y:y,width:this.width,height:fontSize});
  this.showRect = false;
  ...
  this.set = function(ob){  //  use this to change settings for the text
     ...
     if(ob.alpha){
      ...
      if(ob.text){
          ...
          this.rect = new Rectangle({x:this.x,y:this.y,width:this.width,height:this.fontSize,parent:this});
  ...
  this.update = function(){ // redraws the text object
     ...
     // if(newWidth!=this.width){
          ...
          //this.rect = new Rectangle({x:this.x,y:this.y,width:this.width,height:this.fontSize,parent:this});
           this.rect = new Rectangle({x:this.x-2,y:this.y,width:this.width+4,height:this.fontSize+5,parent:this});
  ...
  this.addEventListener = function(event,returnFunction, ob){
      ...
      eventOb.parent = this;
      eventOb.event = event;
      eventOb.returnFunction = returnFunction;
    //  if(debugMode)//console.log( eventOb.returnFunction);
...
this.center.y = this.y+(this.height/2);
  ...
  this.rect = new Rectangle({x:x,y:y,width:this.width,height:this.height});
...
if(text){
  ...
  this.addEventListener = function(event,returnFunction, ob){
      ...
      eventOb.event = event;
      eventOb.returnFunction = returnFunction;
      //console.log( eventOb.returnFunction);
...
function checkRect(ob, x, y){
  var isCollision = false;
      ...
      var left = ob.x;var right = ob.x+ob.rect.width+1;
      var top = ob.y;var bottom = ob.y+ob.rect.height;
      if (right >= x
          && left <= x
          && bottom >= y
          && top <= y) {
...
function collides(rects, x, y) {
  ...
  for (var i = 0, len = rects.length; i < len; i++) {
      var left = rects[i].rect.x;var right = rects[i].rect.x+rects[i].rect.w;
      var top = rects[i].rect.y;var bottom = rects[i].rect.y+rects[i].rect.h;
      if (right >= x
          && left <= x
          && bottom >= y
          && top <= y) {
...
//	mouse.x = e.targetTouches[0].pageX - gameCanvas.offsetLeft;
...
//canvas.width = canvas.width; // erase contents of the page
